...
Game.Launch=function()
{
	...
	'<div class="listing">bread Clicker is a javascript game by <a href="http://orteil.dashnet.org" target="_blank">Orteil</a> and <a href="http://dashnet.org" target="_blank">Opti</a>.</div>'+
	...
	'<div class="listing">&bull; released a low-res retro version of the game, playable here : <a href="http://orteil.dashnet.org/experiments/bread/" target="_blank">orteil.dashnet.org/experiments/bread</a></div>'+
	...
	'<div class="listing">Miss the old version? Your old save was automatically exported <a href="http://orteil.dashnet.org/breadclicker/v10466/" target="_blank">here</a>!</div>'+
	...
	'<div class="listing">&bull; the dungeons beta is now available on <a href="http://orteil.dashnet.org/breadclicker/betadungeons" target="_blank">/betadungeons</a></div>'+
	...
	'<div class="listing">&bull; you can now <a href="http://orteil.dashnet.org/breads2cash/" target="_blank">convert your breads to cash</a>!</div>'+
	...
	'<div class="listing">&bull; dungeons still need a bit more work and will be released soon - we want them to be just right! (you can test an unfinished version in <a href="http://orteil.dashnet.org/breadclicker/betadungeons/" target="_blank">the beta</a>)</div>'+
	
	'</div><div class="subsection update">'+
	'<div class="title">28/09/2013 - dungeon beta</div>'+
	'<div class="listing">&bull; from now on, big updates will come through a beta stage first (you can <a href="http://orteil.dashnet.org/breadclicker/betadungeons/" target="_blank">try it here</a>)</div>'+
	...
	Game.ErrorFrame=function()
	{
		...
		'You can <a href="http://orteil.dashnet.org/breadclicker/" target="_blank">play bread Clicker over here</a>!<br>'+
	...
	Game.Init=function()
	{
		...
		Game.breadsPs=1;//bread per second (to recalculate with every new purchase)
		...
		Game.customMouseCps=[];//add to the breads earned per click computation (functions should return something to add to the multiplier ie. 0.1 for an addition of 10 to the CpS multiplier)
		Game.customMouseCpsMult=[];//add to the breads earned per click multiplicative computation (functions should return something to multiply by the multiplier ie. 1.05 for a 5% increase of the multiplier)
		Game.custombreadClicks=[];//add to the bread click calls
		...
		Game.attachTooltip(l('topbarMobileCC'),'<div style="padding:8px;width:250px;text-align:center;">Play bread Clicker on your phone!<br>(Currently in beta, Android only; iOS version will be released later)</div>','this');
					...
					if (version>=1)
					{
						...
						for (var i in Game.ObjectsById)
						{
							...
							if (spl[i])
							{
								...
								me.amount=parseInt(mestr[0]);me.bought=parseInt(mestr[1]);me.totalbreads=parseFloat(mestr[2]);me.level=parseInt(mestr[3]||0);
							...
							{
								me.amount=0;me.unlocked=0;me.bought=0;me.totalbreads=0;me.level=0;
							}
						}
		...
		Game.Reset=function(hard)
		{
			...
			{
				var me=Game.ObjectsById[i];
				me.amount=0;me.bought=0;me.free=0;me.totalbreads=0;
				me.switchMinigame(false);
				if (hard) {me.muted=0;}
				me.pics=[];
				me.refresh();
		...
		l('ascendOverlay').innerHTML=
			...
			'<div class="ascendData smallFramed prompt" '+Game.getTooltip(
							'<div style="min-width:200px;text-align:center;font-size:11px;">Each prestige level grants you a permanent +1% CpS.<br>The more levels you have, the more breads they require.</div>'
		...
		Game.Clickbread=function(e,amount)
		{
			...
			else
			{
				...
				for (var i in Game.custombreadClicks) {Game.custombreadClicks[i]();}
		...
		for (var i=0;i<50;i++)
		{
			Game.particles[i]={x:0,y:0,xd:0,yd:0,w:64,h:64,z:0,size:1,dur:2,life:-1,r:0,pic:'smallbreads.png',picId:0};
		}
		...
		Game.UpdateMenu=function()
		{
			...
			else if (Game.onMenu=='stats')
			{
				...
				'<div class="listing"><b>bread per second :</b> '+Beautify(Game.breadsPs,1)+' <small>'+
				...
				'</div>':'')+
				(Game.season=='fools'?
					'<div class="listing"><b>Money made from selling breads :</b> $'+Beautify(Game.breadsEarned*0.08,2)+'</div>'+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.breadsEarned>=10100000000)
			{
				...
				if (Math.random()<0.05)
				{
					if (Game.HasAchiev('Base 10')) list.push('News : bread manufacturer completely forgoes common sense, lets OCD drive building decisions!');//somehow I got flak for this one
				...
				choose([
					...
					'News : '+choose(['street','school','nursing home','stadium','new fast food chain','new planet','new disease','flesh-eating virus','deadly bacteria','new species of '+choose(animals),'new law','baby','programming language'])+' to be named after '+Game.bakeryName+', the world-famous bread manufacturer.',
		...
		AddEvent(Game.tickerL,'click',function(event){
			...
			if (Game.TickerEffect && Game.TickerEffect.type=='fortune')
			{
				...
				else if (effect=='fortuneCPS')
				{
					...
					Game.fortuneCPS=1;
					Game.Earn((Game.breadsPs*60*60,Game.breads));
				}
				else
				{
			...
			this.tooltip=function()
			{
				...
				if (me.amount>0)
				{
					...
					if (synergyBoost>0)
					{
						...
						//synergiesStr='...also boosting some other buildings, accounting for <b>'+Beautify(synergyBoost,1)+'</b> bread per second (a combined <b>'+Beautify((synergyBoost/Game.breadsPs)*100,1)+'%</b> of total CpS) : '+synergiesStr+'';
						synergiesStr='...also boosting some other buildings : '+synergiesStr+' - all combined, these boosts account for <b>'+Beautify(synergyBoost,1)+'</b> bread per second (<b>'+Beautify((synergyBoost/Game.breadsPs)*100,1)+'%</b> of total CpS)';
					}
				}
		...
		new Game.Upgrade('Million fingers','The mouse and cursors gain <b>+0.5</b> breads for each non-cursor object owned.<q>clickityclickity</q>',10000000,[0,14]);Game.MakeTiered(Game.last,5,0);
		new Game.Upgrade('Billion fingers','The mouse and cursors gain <b>+5</b> breads for each non-cursor object owned.<q>clickityclickityclickity</q>',100000000,[0,15]);Game.MakeTiered(Game.last,6,0);
		new Game.Upgrade('Trillion fingers','The mouse and cursors gain <b>+50</b> breads for each non-cursor object owned.<q>clickityclickityclickityclickity</q>',1000000000,[0,16]);Game.MakeTiered(Game.last,7,0);
		...
		Game.NewUpgradebread({name:'Peanut butter breads',desc:'Get yourself some jam breads!',icon:[1,3],power:								1,	price:	9999999*5});
		...
		Game.NewUpgradebread({name:'Macadamia nut breads',desc:'They\'re macadamn delicious!',icon:[5,3],power:								2,	price:	999999999});
		...
		order=100;
		new Game.Upgrade('Quadrillion fingers','The mouse and cursors gain <b>+500</b> breads for each non-cursor object owned.<q>clickityclickityclickityclickityclick</q>',10000000000,[0,17]);Game.MakeTiered(Game.last,8,0);
		...
		order=10020;
		Game.NewUpgradebread({name:'Dark chocolate-coated breads',desc:'These absorb light so well you almost need to squint to see them.',icon:[10,3],power:			4,	price:	99999999999});
		Game.NewUpgradebread({name:'White chocolate-coated breads',desc:'These dazzling breads absolutely glisten with flavor.',icon:[11,3],power:					4,	price:	99999999999});
		...
		order=100;
		new Game.Upgrade('Quintillion fingers','The mouse and cursors gain <b>+5000</b> breads for each non-cursor object owned.<q>man, just go click click click click click, it\'s real easy, man.</q>',10000000000000,[0,18]);Game.MakeTiered(Game.last,9,0);
		...
		order=100;
		new Game.Upgrade('Sextillion fingers','The mouse and cursors gain <b>+50000</b> breads for each non-cursor object owned.<q>sometimes<br>things just<br>click</q>',10000000000000000,[0,19]);Game.MakeTiered(Game.last,10,0);
		...
		order=100;
		new Game.Upgrade('Septillion fingers','The mouse and cursors gain <b>+500000</b> breads for each non-cursor object owned.<q>[cursory flavor text]</q>',10000000000000000000,[12,20]);Game.MakeTiered(Game.last,11,0);
		new Game.Upgrade('Octillion fingers','The mouse and cursors gain <b>+5000000</b> breads for each non-cursor object owned.<q>Turns out you <b>can</b> quite put your finger on it.</q>',10000000000000000000000,[12,19]);Game.MakeTiered(Game.last,12,0);
		...
		order=10020;
		Game.NewUpgradebread({name:'Pure black chocolate breads',desc:'Dipped in a lab-made substance darker than the darkest cocoa (dubbed "chocoalate").',icon:[26,3],power:									4,price: 9999999999999999*5});
		Game.NewUpgradebread({name:'Pure white chocolate breads',desc:'Elaborated on the nano-scale, the coating on this biscuit is able to refract light even in a pitch-black environment.',icon:[26,4],power:	4,price: 9999999999999999*5});
		...
		order=10300;
		Game.NewUpgradebread({name:'Milk chocolate butter biscuit',desc:'Rewarded for owning 100 of everything.<br>It bears the engraving of a fine entrepreneur.',icon:[27,8],power:	10,price: 999999999999999999999,locked:1});
		Game.NewUpgradebread({name:'Dark chocolate butter biscuit',desc:'Rewarded for owning 150 of everything.<br>It is adorned with the image of an experienced bread tycoon.',icon:[27,9],power:	10,price: 999999999999999999999999,locked:1});
		Game.NewUpgradebread({name:'White chocolate butter biscuit',desc:'Rewarded for owning 200 of everything.<br>The chocolate is chiseled to depict a masterful pastry magnate.',icon:[28,9],power:	10,price: 999999999999999999999999999,locked:1});
		Game.NewUpgradebread({name:'Ruby chocolate butter biscuit',desc:'Rewarded for owning 250 of everything.<br>Covered in a rare red chocolate, this biscuit is etched to represent the face of a bread industrialist gone mad with power.',icon:[28,8],power:	10,price: 999999999999999999999999999999,locked:1});
		...
		Game.NewUpgradebread({name:'Dim Dams',desc:'Two biscuits joined by chocolate and coated in even more chocolate.<br>You wonder - which one is the dim, and which one is the dam?',icon:[31,10],require:'Box of brand biscuits',power:												2,	price:	999999999999999999999999999999*5});
		
		order=10060;
		Game.NewUpgradebread({name:'Candy',desc:'There are two pillars to the world of sweets : pastries, of course - and candy.<br>You could make a whole new game just about these, but for now, please enjoy these assorted generic candies.',icon:[30,10],require:'Box of not breads',		power:5,price: Math.pow(10,46)});
		...
		Game.ProductionAchievement=function(name,building,tier,q,mult)
		{
			...
			var achiev=new Game.Achievement(name,'Make <b>'+Beautify(pow)+'</b> breads just from '+building.plural+'.'+(q?'<q>'+q+'</q>':''),icon);
		...
		Game.BankAchievement=function(name)
		{
			...
			var achiev=new Game.Achievement(name,'Bake <b>'+Beautify(threshold)+'</b> bread'+(threshold==1?'':'s')+' in one ascension.',[Game.thresholdIcons[Game.BankAchievements.length],(Game.BankAchievements.length>32?1:Game.BankAchievements.length>23?2:5)]);
		...
		Game.CpsAchievement=function(name)
		{
			...
			var achiev=new Game.Achievement(name,'Bake <b>'+Beautify(threshold)+'</b> bread'+(threshold==1?'':'s')+' per second.',[Game.thresholdIcons[Game.CpsAchievements.length],(Game.CpsAchievements.length>32?1:Game.CpsAchievements.length>23?2:5)]);
			achiev.threshold=threshold;
		...
		Game.BankAchievement('He who controls the breads controls the universe');Game.last.baseDesc+='<q>The milk must flow!</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		Game.BankAchievement('In the mouth of madness');Game.last.baseDesc+='<q>A bread is just what we tell each other it is.</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		Game.CpsAchievement('breadhertz : a really, really tasty hertz');Game.last.baseDesc+='<q>Tastier than a hertz donut, anyway.</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		Game.CpsAchievement('Well the breads start coming and they don\'t stop coming');Game.last.baseDesc+='<q>Didn\'t make sense not to click for fun.</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		Game.CpsAchievement('I don\'t know if you\'ve noticed but all these icons are very slightly off-center');
		Game.CpsAchievement('The proof of the bread is in the baking');Game.last.baseDesc+='<q>How can you have any breads if you don\'t bake your dough?</q>';Game.last.desc=BeautifyInText(Game.last.baseDesc);
		...
		*/
		...
		new Game.buffType('building buff',function(time,pow,building)
		{
			...
			return {
				...
				desc:'Your '+obj.amount+' '+obj.plural+' are boosting your CpS!<br>bread production +'+(Math.ceil(pow*100-100))+'% for '+Game.sayTime(time*Game.fps,-1)+'!',
		...
		new Game.buffType('building debuff',function(time,pow,building)
		{
			...
			return {
				...
				desc:'Your '+obj.amount+' '+obj.plural+' are rusting your CpS!<br>bread production '+(Math.ceil(pow*100-100))+'% slower for '+Game.sayTime(time*Game.fps,-1)+'!',
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			Game.Earn(Game.breadsPs/Game.fps);//add bread per second
			...
			//Game.Earn(cps/Game.fps);//add bread per second
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.Has('Eternal heart biscuits')) Game.Win('Lovely breads');
				if (Game.season=='easter')
				{
					var eggs=0;
					for (var i in Game.easterEggs)
